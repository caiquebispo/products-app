<div class="py-12">
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div>
                <livewire:brands.store />
            </div>

            <div class="col-span-1 lg:col-span-2">
                <x-card title="Minhas Marcas">
                                    ...
                                    <tr class="transition-colors duration-150 hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium min-w-[100px]">
                                            <div class="flex items-center justify-end gap-2">
                                                <button 
                                                    wire:click="$dispatch('brands::edit', { id: {{ $brand->id }} })"
                                                ...
                                                <button 
                                                    wire:click="$dispatch('brands::delete', { id: {{ $brand->id }} })"
                                        ...
                                        <td colspan="5" class="min-w-full px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p class="max-w-md text-center text-gray-500">
                                                    @if($search)
                                                        Não encontramos marcas que correspondam à sua busca "{{ $search }}".
                                                    @else
                                                        Comece criando sua primeira marca usando o formulário ao lado.
                                                    @endif
                                                </p>
                                                @if($search)
                                                    <button 
                    ...
                    <div class="flex items-center justify-between mt-4 text-sm text-gray-700">
                        <div>
                            Mostrando {{ $brands->firstItem() ?? 0 }} a {{ $brands->lastItem() ?? 0 }} 
                            de {{ $brands->total() }} {{ $brands->total() === 1 ? 'marca' : 'marcas' }}
                        </div>
                        @if($search)
                            <div class="flex items-center gap-2">
